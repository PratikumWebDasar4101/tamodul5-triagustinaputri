...
$hobi = $_POST['hobi'];
	...
	if (isset($_POST['nama']) AND isset($_POST['email']  )){
   	...
   	header("Location: registrasi.php?error=variabel_belum_diset");
}
	if(empty($nama)){
   	header("Location: registrasi.php?error=nama_kosong");
	}else{
		if (is_numeric($nama)){
     		 header("Location: registrasi.php?error=nama_harus_huruf");
	}else{
		echo "Nim  : $nim" . "<br>";
		echo "Nama : $nama" . "<br>";
